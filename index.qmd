...
The specification of **Data Validation Report Format** (DVRF) has two goals:
...
Applications MAY restrict their support of Data Validation Report Format to positions in condense form being  locator maps.
...
- the locator MAY have a field `value` with the referenced element encoded as JSON object that maps document model names (some included in the the table of [dimensions](#dimensions)) to serializations in some reasonable form (typically as JSON string). The value MUST be derived from the document, dimension, and address. Applications MAY replace the field with another value derived from document, dimension, and address. 
...
- the report SHOULD have field `partial` with a boolean value or an array of [errors] to indicate when the document was fully validated. The errors listed in this array MUST NOT be confused with validation errors found by validation. Instead they refer to errors of the execution of validation process.
...
The **line numbers** locator format with name `lines` is used to reference a sequence of consecutive line (each being a character string) from a sequence of lines. The locator value is two positive integers encoded as string without leading zeroes and separated with minus `-`. The second integer MUST NOT be larger than the first. The first line has number one (locator value `1`).
